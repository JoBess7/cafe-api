...
public class CafeServiceTest {
    ...
    private static final MenuParams A_MENU_PARAMS = new MenuParams("Pumpkin Pie", 2, 20, 20, 10, 10);
    ...
    @Test
    public void whenAddingCoffeeType_shouldGetCafe() {
        when(cafeRepository.get()).thenReturn(A_CAFE);

        cafeService.addCoffeeType(A_MENU_PARAMS);

        verify(cafeRepository).get();
    }

    @Test
    public void whenAddingCoffeeType_shouldUpdateCafe() {
        Cafe mockCafe = mock(Cafe.class);
        when(cafeRepository.get()).thenReturn(mockCafe);
    ...
    @Test
    public void whenAddingCoffeeType_shouldAddCoffeeTypeToCafe() {
        Cafe mockCafe = mock(Cafe.class);
        when(cafeRepository.get()).thenReturn(mockCafe);
        CoffeeType coffeeType = new CoffeeType(A_MENU_PARAMS.name(), A_MENU_PARAMS.cost(), A_MENU_PARAMS.recipe(), false);

        cafeService.addCoffeeType(A_MENU_PARAMS);

        verify(mockCafe).addCoffeeType(coffeeType);
    }
}
